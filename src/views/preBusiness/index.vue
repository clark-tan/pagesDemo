 ...
 * @LastEditTime: 2020-12-21 09:51:08
 -->
...
export default {
  ...
  methods: {
    ...
    GetDataByTemporarilyToCode () {
      this.$WebApi.preBusiness['GetDataByTemporarilyToCode']({
        ...
        .then(res => {
          ...
          console.log('当前步骤')
          console.log(content)
          if (content[content.length - 1].currentComponentName && content[content.length - 1].currentComponentDesc) {
            this.currentComponentName = content[content.length - 1].currentComponentName
            this.currentComponentDesc = content[content.length - 1].currentComponentDesc
            this.current = content[content.length - 1].current
          } else {
            this.currentComponentName = content[0].componentName
            this.currentComponentDesc = content[0].componentDesc
          }
          this.processData = content.slice(0, content.length - 1)
          this.getComponentName(content.slice(0, content.length - 1)) // 函数执行顺序不能变化
        })
        .catch(err => {
          console.log(err)
        })
    ...
    GetPersonAppointmentDetail (submitData) {
      this.$WebApi.busiProcess['GetPersonAppointmentDetail']({
        ...
        .then(res => {
          ...
          temp.REC_LXFS = this.saveData.LXDH
    ...
    handleSubmit () {
      ...
      // 标识当前填写的位置
      if (this.processData[this.current]) {
        submitData.push({
          currentComponentName: this.processData[this.current].componentName,
          currentComponentDesc: this.processData[this.current].componentDesc,
    ...
    handleSaveData (saveData) {
      ...
      this.$WebApi.preBusiness['SubmitEntrust'](formData)
        .then(res => {
          this.saveDataID = res
